...
struct TestBondParameters{T<: Real} <: RosenbluthChains.AbstractBondParam
    ### Bond Length
    BondLength::T
end 

@inline function RosenbluthChains.InitSimParam(data::SimData,param::TestBondParameters ) 
    fill!(data.trial_radius, param.BondLength)
end


@inline function RosenbluthChains.SetTrialRadius(data::SimData, param::TestBondParameters) 
    ### is set once in set up and doesnt change afterwards
end

@inline function RosenbluthChains.GetTrialBoltzmannWeight(data::SimData,param::TestBondParameters) nothing end


KP = SimulationParameters( TestBondParameters(r), FixedBondAngles(θ), FixedTorsionAngles(φ), IdealChain())
